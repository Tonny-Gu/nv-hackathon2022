...
template <typename T>
__device__ inline T log(T a) {
  return ::log(a);
}

template <>
__device__ inline __half log(__half a) {
  return hlog(a);
}
...
template <typename T>
__host__ __device__ inline T float2type(float a) {
  return (T) a;
}

template <>
__host__ __device__ inline __half float2type(float a) {
  return __float2half(a);
}

template <typename T>
__host__ __device__ inline float type2float(T a) {
  return (float) a;
}

template <>
__host__ __device__ inline float type2float(__half a) {
